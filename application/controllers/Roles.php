...
class Roles extends BaseController
{
    ...
    public function store()
    {
        ...
        $this->require_ajax_permission('roles', 'create');
    ...
    public function update()
    {
        ...
        // Permission check handled by BaseController middleware
        $this->require_ajax_permission('roles', 'edit');
    ...
    public function delete()
    {
        ...
        // Permission check handled by BaseController middleware
        $this->require_ajax_permission('roles', 'delete');
    ...
    {
        // Modul dasar wajib ada di setiap peran
        $base = array('dashboard','profile','attendance','leave','announcement');
        $rows = $this->mymodel->selectWithQuery(
            "SELECT id, name FROM modules WHERE name IN ('" . implode("','", $base) . "')");
        foreach (($rows ?? []) as $r) {
            if (!isset($permissions[$r['id']])) $permissions[$r['id']] = array();
            $permissions[$r['id']]['can_view'] = 1;
            if (in_array($r['name'], array('attendance','leave'))) {
                $permissions[$r['id']]['can_create'] = 1;
                $permissions[$r['id']]['can_edit'] = 1;
            }
            if ($r['name'] === 'profile') $permissions[$r['id']]['can_edit'] = 1;
        }
    ...
    private function get_module_permissions()
    {
        return array(
            ...
            'performance_review' => ['view', 'create', 'edit', 'delete', 'approve'],
    ...
    private function get_module_category($module_name)
    {
        $categories = array(
            ...
            'HR Management' => array('quest', 'quest_level', 'position', 'benefit', 'milestone', 'contract', 'performance_review'),
            ...
            'Reports & Analytics' => array('report', 'report_aset', 'expense')
        );
