...
class Influencer extends BaseController
{
    ...
    public function store()
    {
        ...
        if ($exists) {
            $msg = 'Data dengan username tersebut sudah ada di platform yang sama';
            echo $this->template->alert_danger($msg);
            return; 
        }
        ...
        foreach ($list as $kl => $vl) {
            ...
            if ($response['status'] == false) {
                ...
                if ($response['status'] == false) {
                    ...
                    if ($dt['avg_view'] > 0 && $dt['avg_interaksi'] > 0) {
    ...
    public function sync_external_process()
    {
        ...
        $list = $this->mymodel->selectWithQuery("SELECT * FROM influencer WHERE avg_interaksi_2 = 0 AND status = 'Aktif' ");
        ...
        foreach ($list as $kl => $vl) {
            ...
            if ($response['status'] == false) {
                ...
                if ($response['status'] == false) {
                    ...
                    $endorse_dt = array();
                    $endorse_dt['sync_at'] = DATE("Y-m-d H:i:s");
                    $endorse_dt['frequency'] = $endorse['frequency'];
                    $endorse_dt['total_cost'] = $endorse['total_cost'];
                    $endorse_dt['view'] = $endorse['views'];
                    $endorse_dt['like'] = $endorse['likes'];
                    $endorse_dt['comment'] = $endorse['comment'];
                    $endorse_dt['collect'] = $endorse['collect'];
                    $endorse_dt['share'] = $endorse['share'];
                    $endorse_dt['avg_view'] = $endorse['avg_views'];
                    $endorse_dt['avg_interaksi'] = $endorse['avg_interaksi'];
                    if ($endorse['total_cost'] > 0 && $endorse['views'] > 0) {
                        $endorse_dt['cpm'] = $endorse['total_cost'] / $endorse['views'] * 1000;
                    } else {
                        $endorse_dt['cpm'] = 0;
                    }

                    $this->db->update('influencer', $endorse_dt, array('id' => $id));
