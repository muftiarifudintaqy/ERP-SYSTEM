    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="mb-0" style="color: rgba(0, 0, 0, 0.85);">Benefit Management</h5>
                <a href="<?= base_url() ?>/benefit/create_page" class="btn btn-primary">
        ...
        <div class="card-body">
                        ...
                        <div class="input-group" style="box-shadow: 0 2px 0 rgba(0,0,0,0.02);">
                            <div class="dropdown" style="margin-right: 10px;">
                                <button class="btn dropdown-toggle" type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false"
                                    ...
                                    <span style="margin-right: 8px;"><?= $keyword_category ?? 'Tipe' ?></span>
                                </button>
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="position: absolute; background-color: white; min-width: 160px; z-index: 9999; border-radius: 2px; box-shadow: 0 3px 6px -4px rgba(0,0,0,0.12), 0 6px 16px 0 rgba(0,0,0,0.08), 0 9px 28px 8px rgba(0,0,0,0.05); padding: 4px 0; margin-top: 2px; left: 0;">
                                    ...
                                    foreach ($arr as $k => $val) {
                                        $active = (($keyword_category ?? 'Tipe') == $val) ? 'background-color: #e6f7ff; color: #1890ff;' : '';
                                    ?>
                                        <li><a class="dropdown-item" href="<?= base_url() ?>/benefit?keyword_category=<?= $val ?><?= !empty($_GET['keyword']) ? '&keyword=' . $_GET['keyword'] : '' ?>"
                            ...
                            <input type="hidden" name="keyword_category" value="<?= $keyword_category ?? 'Tipe' ?>">
            ...
            <div class="table-responsive">
                <table class="table table-hover" id="benefit-table">
                    <thead>
                        <tr>
...
<script>
    /**
     * Loads benefit data via AJAX
     */
    function loadBenefitData() {
        $.ajax({
            type: 'GET',
            url: "<?= base_url() ?>/benefit/item<?= $param ?>",
            beforeSend: function() {
                $('#benefit-tbody').html('<tr><td colspan="4" class="text-center"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div></td></tr>');
            },
            success: function(data) {
                $('#benefit-tbody').html(data);
            },
            error: function(xhr, status, error) {
                console.error('Error loading data:', error);
                $('#benefit-tbody').html('<tr><td colspan="4" class="text-center text-danger">Error loading data. Please try again.</td></tr>');
            }
        });
    }
