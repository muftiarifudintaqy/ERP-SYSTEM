...
<form action="<?= base_url() ?>/customer/update" method="POST" id="form-modal" enctype="multipart/form-data">
	...
	<div class="col-md-12">
		<label for="">Tag</label>
		<select type="text" class="form-control select2" name="tag[]" multiple="multiple">
			<?php
			foreach ($tag as $k2 => $v2) {
				$text = '';
				if ($data['tag'] == $v2['id']) {
					$text = 'selected';
				}
			?>
				<option <?= $text ?> value="<?= $v2['title'] ?>"><?= $v2['title'] ?></option>
			<?php } ?>
		</select>
	</div>
	<div class="col-md-12">
