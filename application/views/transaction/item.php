...
if ($view == 'table'):
  $this->load->view('transaction/item_table', $data);
?>
...
<?php
  ...
  // Card view
    foreach ($data as $v) {
        ...
        $return_good_qty = floatval($v['return_good_qty'] ?? 0);
        $return_bad_qty = floatval($v['return_bad_qty'] ?? 0);
        $return_condition_items = is_array($v['return_condition_items'] ?? null) ? $v['return_condition_items'] : [];
        $is_return_order = in_array(strtoupper(strval($v['order_status'] ?? '')), ['RETURN', 'RETURN_UNSHIPPED'], true);
    ?>
        <div class="card mb-3">
            <div class="row">
                <div class="col-lg-7">
                    <input class="d-none" type="text" id="box-order-id-<?= $v['id'] ?>" value="<?= $v['order_id'] ?>">
                    <p class="mb-1"><a href="<?= base_url() ?>crm/detail?id=<?= $v['customer'] ?>" class="a-none text-blue fw-700 fs-16">#<?= $k + 1 ?> | <?= $v['order_id'] ?></a> <a href="#!" onclick="copy('<?= $v['id'] ?>')"><i class="bi bi-copy"></i></a><?php if (!empty($v['is_for_booking']) && (int)$v['is_for_booking'] === 1) { ?> <span class="bg-blue br-10 fs-12 text-white ms-1">Booking</span><?php } ?></p>
                    ...
                    <?php } ?>
                    <p class="mb-1" id="order_status-<?= $v['id'] ?>"><span class="<?= $order_class ?> br-10 fs-12 text-white"><?= $v['order_status'] ?></span>
                        <?php if ($is_return_order) { ?>
                            <?php foreach ($return_condition_items as $return_item) {
                                $product_name = trim(str_replace('amp;', '', $return_item['product_text'] ?? ''));
                                if ($product_name === '') {
                                    $product_name = trim($return_item['sku'] ?? 'Produk');
                                }
                                $good_qty = floatval($return_item['return_good_qty'] ?? 0);
                                $bad_qty = floatval($return_item['return_bad_qty'] ?? 0);
                            ?>
                                <?php if ($good_qty > 0) { ?>
                                    <span class="bg-green br-10 fs-12 text-white ms-1">Good <?= number_format($good_qty, 0, ',', '.') ?>x <?= htmlspecialchars($product_name, ENT_QUOTES, 'UTF-8') ?></span>
                                <?php } ?>
                                <?php if ($bad_qty > 0) { ?>
                                    <span class="bg-red br-10 fs-12 text-white ms-1">Bad <?= number_format($bad_qty, 0, ',', '.') ?>x <?= htmlspecialchars($product_name, ENT_QUOTES, 'UTF-8') ?></span>
                                <?php } ?>
                            <?php } ?>
                            <?php if (empty($return_condition_items) && ($return_good_qty > 0 || $return_bad_qty > 0)) { ?>
                                <?php if ($return_good_qty > 0) { ?>
                                    <span class="bg-green br-10 fs-12 text-white ms-1">Good <?= number_format($return_good_qty, 0, ',', '.') ?>x</span>
                                <?php } ?>
                                <?php if ($return_bad_qty > 0) { ?>
                                    <span class="bg-red br-10 fs-12 text-white ms-1">Bad <?= number_format($return_bad_qty, 0, ',', '.') ?>x</span>
                                <?php } ?>
                            <?php } ?>
                        <?php } ?>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4 mb-3">
                            <?php if ($v['is_manual'] == 0) { ?>
                                ...
                                <?php
                                // Kunci name_parent dan name tidak pernah ada di
                                // data Shopee maupun TikTok, jadi baris produk
                                // selalu kosong. Yang terisi model_name,
                                // item_name, dan model_sku. Gambar produk
                                // ditampilkan sebagai gambar kecil di samping
                                // namanya supaya barangnya langsung dikenali.
                                foreach ((json_decode($v['pesanan'], true) ?: []) as $k2 => $v2) {
                                    $nama = '';
                                    foreach (['model_name', 'item_name', 'model_sku'] as $kk) {
                                        if (!empty($v2[$kk])) { $nama = trim((string)$v2[$kk]); break; }
                                    }
                                    if ($nama === '') continue;

                                    $baris = ((int)($v2['qty'] ?? 1)) . 'x ' . str_replace('amp;', '', $nama);
                                    if (!empty($v2['model_sku'])) {
                                        $baris .= ' | ' . $v2['model_sku'];
                                    }
                                    $gbr = $v2['image_url'] ?? '';
                                ?>
                                    <div class="d-flex align-items-center gap-2 mb-2">
                                        <?php if ($gbr): ?>
                                            <img src="<?= html_escape($gbr) ?>" loading="lazy" alt=""
                                                 style="width:40px;height:40px;border-radius:6px;
                                                        object-fit:cover;flex:0 0 40px;background:#f1f1f4"
                                                 onerror="this.style.display='none'">
                                        <?php endif; ?>
                                        <span class="a-none text-blue fw-700 fs-16"><?= $baris ?></span>
                                    </div>
                                <?php  } ?>
                            <?php } else { ?>
                                <p class="mb-1 fs-16">Produk (Total <?= $v['pesanan_count'] ?> item)</p>
                ...
                <div class="col-lg-12 pb-2">
                    <div class="row">
                        <div class="col-lg-6">
                            ...
                            <input class="checkItem" style="top:10px" type="checkbox" value="<?= $v['id'] ?>" data-id="<?= $k ?>" data-status="<?= htmlspecialchars($v['order_status'], ENT_QUOTES, 'UTF-8') ?>" name="list_id" form="form-action">
                            </div>
