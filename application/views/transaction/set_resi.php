...
<form action="<?= base_url() ?>/transaction/set-resi-process" method="POST" id="form-modal">
	<input type="hidden" name="id" value="<?= $data['id'] ?>">
	<p>Apakah kamu yakin ingin mengubah no resi order ini?</p>
    <input type="text" value="<?=$data['awb_number']?>" name="dt[awb_number]" class="form-control" id="awb_number">
			...
			success: function(response, textStatus, xhr) {
				...
				if (str.indexOf("success") != -1) {
					...
					setTimeout(function() {
						...
						$("#awb_number-<?=$data['id']?>").html($("#awb_number").val());
