	...
	<div class="card">
		<div class="card-header">
			<div class="d-flex justify-content-between align-items-center">
				<h5 class="mb-0" style="color: rgba(0, 0, 0, 0.85);">Ekspedisi Management</h5>
						...
						<div class="input-group">
							<div class="dropdown" style="margin-right: 10px;">
								...
								<ul class="dropdown-menu">
									<?php
										$arr = array('Nama Ekspedisi', 'Keterangan');
										foreach ($arr as $val) {
...
<script>
	function create() {
		...
		$("#load-form").load("<?= base_url() ?>/shipping/create");
	}
	...
	function remove(id) {
		...
		$("#load-form").load("<?= base_url() ?>/shipping/remove?id=" + id);
	}
	...
	function edit(id) {
		...
		$("#load-form").load("<?= base_url() ?>/shipping/edit?id=" + id);
	}
</script>
...
<script>
	function loadMoreData() {
		$.ajax({
			type: 'GET',
			url: "<?= base_url() ?>/shipping/item<?= $param ?>",
			success: function (data) {
				$('#tbody').append(data);
				if (typeof select3 === 'function') { select3(); }
			},
