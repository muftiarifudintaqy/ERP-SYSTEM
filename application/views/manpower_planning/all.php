    ...
    <div class="card">
        <div class="card-header">
            <div class="d-flex justify-content-between align-items-center">
                <h5 class="mb-0" style="color: rgba(0, 0, 0, 0.85);">Manpower Planning</h5>
                <a href="<?= base_url() ?>/manpower_planning/create_page" class="btn btn-primary">
        ...
        <div class="card-body">
                        ...
                        <div class="input-group" style="box-shadow: 0 2px 0 rgba(0,0,0,0.02);">
                            <div class="dropdown" style="margin-right: 10px;">
                                <button class="btn dropdown-toggle" type="button" id="dropdownMenuButton" data-bs-toggle="dropdown" aria-expanded="false"
                                    ...
                                    <span style="margin-right: 8px;"><?= $keyword_category ?? 'Posisi' ?></span>
                                </button>
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="position: absolute; background-color: white; min-width: 160px; z-index: 9999; border-radius: 2px; box-shadow: 0 3px 6px -4px rgba(0,0,0,0.12), 0 6px 16px 0 rgba(0,0,0,0.08), 0 9px 28px 8px rgba(0,0,0,0.05); padding: 4px 0; margin-top: 2px; left: 0;">
                                    ...
                                    foreach ($arr as $k => $val) {
                                        $active = (($keyword_category ?? 'Posisi') == $val) ? 'background-color: #e6f7ff; color: #1890ff;' : '';
                                    ?>
                                        <li><a class="dropdown-item" href="<?= base_url() ?>/manpower_planning?keyword_category=<?= $val ?><?= !empty($_GET['keyword']) ? '&keyword=' . $_GET['keyword'] : '' ?>"
                            ...
                            <input type="hidden" name="keyword_category" value="<?= $keyword_category ?? 'Posisi' ?>">
            ...
            <div class="table-responsive">
                <table class="table table-hover" id="manpower-table">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">Approval CEO</th>
...
<script>
    /**
     * Loads manpower planning data via AJAX
     */
    function loadManpowerData() {
        $.ajax({
            type: 'GET',
            url: "<?= base_url() ?>/manpower_planning/item<?= $param ?>",
            beforeSend: function() {
                $('#manpower-tbody').html('<tr><td colspan="9" class="text-center"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div></td></tr>');
            },
            success: function(data) {
                $('#manpower-tbody').html(data);
            },
            error: function(xhr, status, error) {
                console.error('Error loading data:', error);
                $('#manpower-tbody').html('<tr><td colspan="9" class="text-center text-danger">Error loading data. Please try again.</td></tr>');
            }
        });
    }
    ...
    // Inline edit for approval & progress (like recruitment item)
    $(document).on('click', '.editable-manpower', function(e) {
        if ($(e.target).hasClass('btn-cancel-manpower') || $(e.target).is('select')) {
            return;
        }
    ...
    $(document).on('click', '.btn-cancel-manpower', function(e) {
        e.stopPropagation();
        $(this).closest('.edit-mode').addClass('d-none');
        $(this).closest('.editable-manpower').find('.display-mode').removeClass('d-none');
    });

    $(document).on('change', '.editable-manpower select', function() {
        const cell = $(this).closest('.editable-manpower');
        const id = cell.data('id');
        const field = cell.data('field');
        const newValue = $(this).val();

        $.ajax({
            url: "<?= base_url() ?>/manpower_planning/update_status",
            method: 'POST',
            dataType: 'json',
            data: {
                id: id,
    ...
    function removeManpower(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "<?= base_url() ?>/manpower_planning/delete",
                    data: { id: id },
                    success: function(response) {
