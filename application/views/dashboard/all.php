...
<div class="w-100">
    <div class="row align-items-center">
        <div class="col-lg-12 mb-3">
            ...
            <div class="row">
                <div class="col-md-12">
                    <h3 class="text-primary fw-500">DASHBOARD ORDER MONTERA GROUP</h3>
                </div>
            </div>
        </div>
                ...
                <div class="grafik-content" style="display: none;">
                    <div class="col-lg-12 mt-3 mb-3">
                        <div class="card h-100  summary mb-2">
                            ...
                            <div class="col-md-12">
                                <p class="fw-400 mb-2">Menampilkan metrik: <strong>Penjualan Bersih</strong></p>
                            </div>
                            <div id="get-chart">
                            ...
                            success: function(data) {
                                ...
                                $.ajax({
                                    ...
                                    success: function(netSalesData) {
                                        ...
                                        const totalExpenseAll = Math.round(parseFloat(data.total_expense_all_cat)) || 0;
                                        ...
                                        const baseItems = [
                                            { key: '__ads', label: 'Ads', value: totalAds, pct: pct(totalAds, netSales) },
                                            { key: '__kol', label: 'KOL', value: totalKol, pct: pct(totalKol, netSales) },
                                        ];
                                        const getSection = (key, label) => {
                                            if (sectionMap[key]) return sectionMap[key];
                                            if (key === '__ads' || key === '__kol') return 'marketing';
                                        ...
                                        for (const item of baseItems) {
                                            const section = getSection(item.key, item.label);
                                            if (section === 'marketing') marketingItems.push(item);
                                            else categoryItems.push(item);
                                        }
                                        for (const row of catRows) {
                                            const cat = row.category || 'Lain-lain';
                                            const section = getSection(cat, cat);
                                            const val = Math.round(parseFloat(row.total_spend) || 0);
                                            const pNet = pct(val, netSales);
                                            const item = { key: cat, label: cat, value: val, pct: pNet, isExpense: true };
                                            if (section === 'marketing') marketingItems.push(item);
                                            else categoryItems.push(item);
                                        }
                                        const marketingTotal = marketingItems.reduce((acc, row) => acc + (Math.round(parseFloat(row.value) || 0)), 0);
                                        const allSpend = totalAds + totalKol + totalExpenseAll;

                                        const adsPercentage = pct(totalAds, netSales);
                                        const kolPercentage = pct(totalKol, netSales);
                                        const marketingPercentage = pct(marketingTotal, netSales);
                                        const totalPercentage = pct(allSpend, netSales);

                                        // ====== (BARU) Render kategori dinamis ======
                                        const filteredCatRows = categoryItems;
                                        // Batasi 6 teratas agar tooltip tetap ringkas
                                        const MAX_ROWS = 6;
                                        let shown = 0;
                                        let categoryHtml = '';
                                        for (const row of filteredCatRows) {
                                            if (shown >= MAX_ROWS) break;
                                            const name = row.label || 'Lain-lain';
                                            const val = Math.round(parseFloat(row.value) || 0);
                                            const pNet = row.pct !== undefined ? row.pct : pct(val, netSales);
                                            categoryHtml += `
                                        <div class="cat-row" style="display:flex;justify-content:space-between;gap:8px;margin-bottom:4px;">
                                        <span class="text-secondary">${escapeHtml(name)}</span>
                                        <span style="display:flex;justify-content:flex-end;gap:10px;min-width:170px;">
                                            <span class="text-danger" style="min-width:90px;text-align:right;">-${formatCurrency(val)}</span>
                                            <span class="text-muted ms-1" style="min-width:70px;text-align:right;">(${pNet}%)</span>
                                        </span>
                                        </div>
                                    `;
                                            shown++;
                                        }
                                        if (filteredCatRows.length > MAX_ROWS) {
                                            const remaining = filteredCatRows.length - MAX_ROWS;
                                    ...
                                    <div class="report-card p-2" style="min-width:320px;line-height:1.5;font-size:13px;">
                                        <div style="display:flex;justify-content:space-between;gap:8px;font-size:15px;">
                                            <span class="fw-bold">Total Pengeluaran</span>
                                            <span style="display:flex;justify-content:flex-end;gap:10px;min-width:170px;">
                                                <span class="text-danger fw-bolder" style="min-width:90px;text-align:right;font-size:15px;">-${formatCurrency(allSpend)}</span>
                                                <span class="text-muted" style="min-width:70px;text-align:right;">(${totalPercentage}%)</span>
                                            </span>
                                        </div>
                                        ...
                                        <div style="display:flex;justify-content:space-between;gap:8px;font-size:14px;">
                                            <span class="fw-bold" style="margin-bottom:4px;">Marketing</span>
                                            <span style="display:flex;justify-content:flex-end;gap:10px;min-width:170px;">
                                                <span class="text-danger fw-bolder" style="min-width:90px;text-align:right;font-size:14px;">-${formatCurrency(marketingTotal)}</span>
                                                <span class="text-muted" style="min-width:70px;text-align:right;">(${marketingPercentage}%)</span>
                                            </span>
                                        </div>
                                        <div class="drop-zone" data-section="marketing" style="padding-left:8px;margin-bottom:4px;">
                                            ${marketingItems.map((row) => {
                                                const pNet = row.key === '__ads' ? adsPercentage : (row.key === '__kol' ? kolPercentage : row.pct);
                                                return `
                                            <div class="cat-row" style="display:flex;justify-content:space-between;gap:8px;margin-bottom:5px;">
                                                <span class="text-secondary fw-bold">${escapeHtml(row.label)}</span>
                                                <span style="display:flex;justify-content:flex-end;gap:10px;min-width:170px;">
                                                    <span class="text-danger fw-bold" style="min-width:90px;text-align:right;">-${formatCurrency(row.value)}</span>
                                                    <span class="text-muted" style="min-width:70px;text-align:right;">(${pNet}%)</span>
                                                </span>
                                            </div>
                                            `;
                                            }).join('')}
                                        ...
                                        <div class="drop-zone" data-section="category">
                                            ${categoryHtml}
                                        </div>
                                    ...
                                    error: function() {
                                        ...
                                        for (const row of catRows) {
                                            ...
                                            const section = getSection(cat, cat);
                                            const val = Math.round(parseFloat(row.total_spend) || 0);
                                            const item = { key: cat, label: cat, value: val, isExpense: true };
                                            if (section === 'marketing') marketingItems.push(item);
                                            else categoryItems.push(item);
                                        }
                                        const totalMarketing = marketingItems.reduce((acc, row) => acc + (Math.round(parseFloat(row.value) || 0)), 0);
                                        const allSpend = totalAds + totalKol + totalExpenseAll;
                                    ...
                                    <div class="report-card p-2" style="min-width:320px;line-height:1.5;font-size:13px;">
                                        ...
                                        <div style="display:flex;justify-content:space-between;gap:8px;font-size:14px;">
                                            ...
                                            <span class="text-danger fw-bolder" style="min-width:90px;text-align:right;font-size:14px;">-${formatCurrency(totalMarketing)}</span>
                                        </div>
                                        <div class="drop-zone" data-section="marketing" style="padding-left:8px;margin-bottom:4px;">
                                            ${marketingItems.map((row) => {
                                                return `
        ...
        <style>
            ...
            .tippy-box .cat-row { cursor: default; }
        </style>
